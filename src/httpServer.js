...
var fs_exists = function(file) { 
  return new Promise(resolve => fs.exists(file, (exists) => {
...
class Server {
  ...
  static onRequest(req, res) {
    try {
      ...
      log(`request ${req.method} ${sPath}  ${fileversion ? "[version= " + fileversion + "]" : ""}`)
  ...
  static PUT(repositorypath, filepath, req, res) {
    ...
    req.on('end', async () => {
      if (fullpath.match(/\/$/)){
        ...
        fs.writeFile(fullpath, fullBody, (fullpath.match(isTextRegEx) ? undefined : "binary"), (err) => {
          if (err) {
            // throw err;
            log(err);
            return;
  ...
  static listVersions(repositorypath, filepath, res) {
    ...
    var format = '\\{\\"version\\":\\"%h\\",\\"date\\":\\"%ad\\",\\"author\\":\\"%an\\"\\,\\"comment\\":\\"%f\\"},'
    respondWithCMD(
      `cd ${repositorypath}; 
      echo "{ \\"versions\\": [";
  ...
  static async getVersion(repositorypath, filepath) {
    return (await run(`cd "${repositorypath}"; git log -n 1 --pretty=format:%H -- "${filepath}"`)).stdout;
  }

  static async getLastModified(repositorypath, filepath) {
    return (await run(`cd "${repositorypath}"; find "${
                       filepath}" -not -path '*/.git/*' -printf "%TY-%Tm-%Td %TH:%TM:%.2TS"`)).stdout;
  }
  ...
  static async GIT(sPath, req, res, cb) {
    ...
    if (sPath.match(/\/_git\/sync/)) {
      ...
      if (RepositoryInSync[repository]) {
        return respondWithCMD("echo Sync in progress: " + repository, res, dryrun);
      }
      RepositoryInSync[repository] = true;
      cmd = `${server}/bin/lively4sync.sh '${lively4DirUnix + "/"+ repository}' '${
